...
@Component({
  ...
  template: `
    <section class="listing">
      <img class="listing-gambarSampul" [src]="bookLocation.gambarSampul" alt="Exterior gambarSampul of {{ bookLocation.judul }}">
      <h2 class="listing-heading">{{ bookLocation.judul }}</h2>
      <p class="listing-location">{{ bookLocation.penulis }}, {{ bookLocation.tahunTerbit }}</p>
      <a [routerLink]="['detail', bookLocation.id]">Show More</a>
    </section>
  `,
